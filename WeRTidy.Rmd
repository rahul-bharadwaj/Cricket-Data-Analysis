---
title: "WeRTidy - Datset and Project Overview"
...
- We use a user-defined function to tidy up the data as follows:
...
- We next write the tidied datasets into CSV files for convenient veiwing of the data on any OS:
```{r WriteCSV}
...
bat_bowl <- top_bat_data %>% filter(Matches>100) %>% 
  ...
  select(Player,efficiency,diff,Batting_Average,Batting_Quality) 

plot_batbowl <- bat_bowl %>% 
  ggplot(aes(y=Batting_Average,x=efficiency,color=Player))+
