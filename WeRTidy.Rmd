...
```{r BatsmenPlots, echo=FALSE, fig.height=12, fig.width=8}
#Virat Kohli
...
```{r KeepersPlots, echo=FALSE, fig.height=3, fig.width=9, message=FALSE, warning=FALSE}
#dismissals
...
```{r Grid3, warning=FALSE, echo=FALSE, message=FALSE, fig.align='center', fig.height=3, fig.width=9}
grid.arrange(keepdismissal, keepcaught, keepstump, keepability)
```

```{r Captain, fig.height=3, fig.width=6}
captain %>%
...
followinglist <- read_csv(here::here("Data/followinglist.csv"))
```
...
```{r NetworkPlot, fig.width=8, echo=FALSE}
set.seed(5556677)
p <- ggplot(data = network,
 aes(from_id = user,
 to_id = screen_name)) +
...
```{r}
rbat <- rpart(BattingQuality ~ NetPerformance, data = top_bat_data)
rpart.plot(rbat)
